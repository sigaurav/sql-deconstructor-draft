import re
import sqlparse
from sqlparse.sql import IdentifierList, Token, Parenthesis, Identifier, Function
...
class SQLDeconstructor:
    ...
    def process_query(self, full_query):
        ...
        :param full_query: input SQL Query
        :return:
        """
        # Replace all extra cols with
        full_query = re.sub(r"(?:'[^']*'\s*)|(\s+)", lambda match: " " if match.group(1) else match.group(0),
                            full_query)
        sql_tokens = sqlparse.parse(full_query)[0].tokens
        identifiers_ = list(IdentifierList(sql_tokens).get_identifiers())

        self.extract_from_table_and_aliases(full_query, is_within_from=False)
        self.extract_select_columns_and_aliases(identifiers_)

        _lookup_dict = self.lookup_table_aliases(self.parent_dict['SELECT'].copy(), self.parent_dict['FROM'].copy())

        # print(_lookup_dict)

    @staticmethod
    def lookup_table_aliases(select_dict, from_dict):
        """
        Method to apply lookup b/w FROM and SELECT and replace Aliases in the select statement -- and map logic (if any)
        :param select_dict: Dictionary with all attributes present in SELECT statement (KEY: Alias, Value: Column Value)
        :param from_dict: Dictionary with all Tables present in FROM. ()
        :return:
        """
        result_dict = {}
        for table_alias in from_dict:
            for col_alias in select_dict:
                select_dict[col_alias] = str(select_dict[col_alias]).replace(table_alias, from_dict[table_alias])

        return select_dict

    def extract_select_columns_and_aliases(self, identifiers_):
        """
        Method to extract column names with their aliases from SQL statement
        :param identifiers_:
        :param sql_query: input SQL statement
        :return:
        """
        ...
        for index, token in enumerate(identifiers_):
            ...
            if isinstance(token, Parenthesis) and is_within_select:
                ...
                self.parent_dict['SELECT'].update(self.identifierlist(token, 'SELECT'))
            elif isinstance(token, Identifier) and is_within_select:
                self.parent_dict['SELECT'].update(self.identifier(token, 'SELECT')[0])

    def extract_from_table_and_aliases(self, full_query, is_within_from=False):
        """
        Method to extract tables names with their aliases from SQL statement
        :param is_within_from: Flag to check if Identifier is present withing FROM statement.
        :param full_query: input SQL statmement
        :return:
        """
        sql_tokens = sqlparse.parse(full_query)[0].tokens
        ...
        for index, token in enumerate(identifiers_):
            ...
            if isinstance(token, Token) and token.value.upper() == "FROM":
                is_within_from = True
                continue
            elif isinstance(token, Token) and token.value.upper() in ['WHERE', 'GROUP BY', 'HAVING']:
            ...
            if isinstance(token, Parenthesis) and is_within_from:
                ...
                self.parent_dict['FROM'].update(self.identifierlist(token))
            elif isinstance(token, Identifier) and is_within_from:
                self.parent_dict['FROM'].update(self.identifier(token)[0])

    def identifierlist(self, tokens, statement_type='None'):
        """
        Method to process values of class sqlparse.sql.IdentifierList
        :param statement_type: Value to identify position of tokens in SQL statement (Ex, FROM, SELECT)
        :param tokens: token which contains values of sqlparse.sql.IdentifierList
        :return:
        """
        temp_dict = {}
        for _token in tokens.get_identifiers():
            temp_dict.update(self.identifier(_token, statement_type)[0])
        return temp_dict

    def identifier(self, token, statement_type='None'):
        """
        Method to process values of class sqlparse.sql.Identifier
        :param statement_type: Value to identify position of tokens in SQL statement (Ex, FROM, SELECT)
        :param token: token which contains values of sqlparse.sql.IdentifierList
        :return:
        """
        temp_dict = {}
        without_alias = []
        if statement_type == 'SELECT':
            token_alias, token_name, table_name = self.get_values(token)
            print(token_alias, token_name, table_name)
            exit(0)
        if isinstance(token[0], Function):
            print(token)
            exit(0)
        if token.has_alias():
            temp_dict[token.get_alias()] = token.value.replace(token.get_alias(), '').strip()
        else:
            without_alias = [1]

        return temp_dict, without_alias
    ...
    def get_values(self, token):
        ...
        token_column = self.get_column_names(token)

        return alias, 'b', 'c'

    def get_column_names(self, token):
        """
